...
public class PPImageView extends AppCompatImageView {
    ...
    public static void setImageUrl(PPImageView view, String imageUrl, boolean isCircle, int radius) {
        ...
        if (isCircle) {
            ...
            builder.transform(new RoundedCornersTransformation(PixUtils.dp2pix(radius), 0));
        }
    ...
    private void setSize(int width, int height, int marginLeft, int maxWidth, int maxHeight) {
        ...
        if (params instanceof FrameLayout.LayoutParams) {
            ((FrameLayout.LayoutParams) params).leftMargin = height > width ? PixUtils.dp2pix(marginLeft) : 0;
        } else if (params instanceof LinearLayout.LayoutParams) {
            ((LinearLayout.LayoutParams) params).leftMargin = height > width ? PixUtils.dp2pix(marginLeft) : 0;
        }
